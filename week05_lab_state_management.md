...
<img width="1919" height="1018" alt="image" src="https://github.com/user-attachments/assets/00d79532-5d42-44b7-8a8a-0ee513e9a853" />
<img width="1919" height="1011" alt="image" src="https://github.com/user-attachments/assets/1f7410ee-2157-4ce5-9a5a-c5b351eaa8e6" />
...
```text
จะเกิดปัญหาที่ไม่สามารถส่งข้อมูล `savedItems` และฟังก์ชัน `_onSave` ข้าม Route ไปยังหน้า `FavoritesPage` ผ่าน Constructor ธรรมดาได้ เนื่องจาก Navigator (Route) แยกออกจาก Widget Tree ปกติ ทำให้ต้องส่งผ่านพารามิเตอร์ยุ่งยากมากหรือทำไม่ได้ และข้อมูลจะไม่ซิงค์กันแบบเรียลไทม์
```
...
```text
นี่คือตัวอย่างผลลัพธ์การวิเคราะห์ตาม Prompt ในขั้นตอนที่ 3.1 สำหรับนำไปบันทึกในใบงานครับ:

* **1. Dark Mode / Light Mode (ส่งผลต่อทุกหน้าจอในแอป)**
* **เครื่องมือที่แนะนำ:** Provider หรือ Riverpod
* **เหตุผล:** จัดเป็น App State ที่ต้องแชร์และส่งผลต่อ Widget ทั่วทั้งแอปพลิเคชัน จำเป็นต้องใช้ตัวจัดการสถานะส่วนกลางเพื่อให้ทุกหน้าจอเปลี่ยนธีมตามพร้อมกันได้


* **2. ตัวนับ "มีคนถูกใจแล้วกี่คน" (ซิงค์ระหว่างหน้ารายการกับหน้ารายละเอียดสินค้า)**
* **เครื่องมือที่แนะนำ:** Provider หรือ Riverpod
* **เหตุผล:** เป็น App State ที่ต้องใช้งานและซิงค์ข้อมูลร่วมกันข้ามหน้าจอ (Cross-route) เพื่อให้ค่าความถูกใจอัปเดตตรงกันทันทีไม่ว่าจะอยู่หน้าใด


* **3. Animation กระพริบของไอคอนหัวใจตอนกดถูกใจ (ใช้เฉพาะใน widget เดียว)**
* **เครื่องมือที่แนะนำ:** setState
* **เหตุผล:** จัดเป็น Ephemeral State ที่มีความหมายเฉพาะภายใน Widget เดียวหรือแอนิเมชันย่อย ไม่มีหน้าจออื่นต้องรับรู้ข้อมูลส่วนนี้ จึงใช้เครื่องมือที่เรียบง่ายที่สุดอย่าง `setState` ได้ทันทีโดยไม่ต้องพึ่ง Global State
```
...
```text
* **Gemini แนะนำตรงกับกรอบการตัดสินใจในบทเรียนหรือไม่:**
คำแนะนำของ Gemini **ตรงกัน** กับกรอบการตัดสินใจในบทเรียน เนื่องจากมีการใช้หลักการแบ่งประเภทของ State ได้อย่างถูกต้องตามหลักสากล (แยกระหว่าง Ephemeral State และ App State)
* **มีจุดใดที่ต่างกัน:**
ไม่มีจุดที่ขัดแย้งกันอย่างมีนัยสำคัญ คำแนะนำของ AI สอดคล้องกับแนวคิดในบทเรียนทุกประการ คือ ฟีเจอร์ที่ทำงานเฉพาะภายใน Widget เดียวเลือกใช้ `setState` ส่วนฟีเจอร์ที่ต้องแชร์ข้ามหน้าจอหรือส่งผลต่อทั้งแอปเลือกใช้ `Provider` (หรือ `Riverpod`)
```
...
```text
เหตุผลการเลือกชนิด State:

ฟีเจอร์ช่องค้นหาสินค้า (Search Box): เลือกใช้ Ephemeral State (setState) เนื่องจากสถานะการพิมพ์คำค้นหาใช้งานและมีผลเฉพาะภายในหน้า Home เท่านั้น ไม่มีหน้าจออื่นจำเป็นต้องรับรู้ค่านี้ การใช้เครื่องมือที่เรียบง่ายจึงเหมาะสมที่สุด
